...
- Contract tests (including the real-proof fixture test above) run in CI on every push that touches `circom/contracts/`, on any branch, via [`.github/workflows/circom-contracts-tests.yml`](../../../../.github/workflows/circom-contracts-tests.yml). Example passing `run_contracts_tests` job: [`milestone-3/manifest.json#L12-L16`](../milestone-3/manifest.json#L12-L16). For the current state, see the [Actions tab](https://github.com/zkemail/sdk-images/actions/workflows/circom-contracts-tests.yml?query=branch%3Astaging).
- The Rust tooling that generates the wrapper/verifier (`prepare_contract_data` in `circom/src/contract.rs`) is unit-tested in CI on every push and pull request to `main` and `staging` via [`.github/workflows/circom-rust-tests.yml`](../../../../.github/workflows/circom-rust-tests.yml). Example passing `test` job: [`milestone-3/manifest.json#L7-L11`](../milestone-3/manifest.json#L7-L11). For the current state, see the [Actions tab](https://github.com/zkemail/sdk-images/actions/workflows/circom-rust-tests.yml?query=branch%3Astaging).
- CI runs on the branches that carry the code (`main` and `staging`); this `kusama-grant` branch differs from `staging` in documentation only, so the runs linked above are the corresponding `staging` runs.
