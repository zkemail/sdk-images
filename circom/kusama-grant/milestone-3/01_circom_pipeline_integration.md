...
**CI:** `TBD` -- not yet covered by [`circom-rust-tests.yml`](../../../.github/workflows/circom-rust-tests.yml) (currently scoped to the `contract::` module); these tests live under `payload::` and the crate-root `tests` module. Add a passing run link once the workflow covers them.
